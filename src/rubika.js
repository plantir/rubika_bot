...
const RubikaBotPolling = require('./rubikaPolling');
const debug = require('debug')('rubika-bot');
...
class Rubikabot extends EventEmitter {
  /**
  ...
  constructor(token, options = {}) {
    ...
    this.options.baseApiUrl = options.baseApiUrl || 'https://botapi.rubika.ir';
    ...
    if (options.polling) {
      ...
      if (typeof autoStart === 'undefined' || autoStart === true) {
        this.startPolling(this.options);
      }
    }
  ...
  _request(_path, options = {}) {
    ...
    options.method = 'POST';
    options.body.method = _path;
    options.url = this.options.baseApiUrl; //this._buildURL(_path);
    ...
    return request(options)
      .then(resp => {
        ...
        throw new errors.TelegramError(`${resp.status} ${resp.status_det}`, resp);
      }).catch(error => {
  ...
  startPolling(options = {}) {
    ...
    if (!this._polling) {
      this._polling = new RubikaBotPolling(this);
    }
   ...
   */
  getMessages(body = {}) {
    return this._request('getMessages', {
      body
    });
  }
    ...
    if (text) {
      ...
      if (text) {
        ...
        this._textRegexpCallbacks.some(reg => {
          debug('Matching %s with %s', text, reg.regexp);
   ...
   * @see https://core.telegram.org/bots/api#sendmessage
   */
  sendMessage(chatId, text, body = {}) {
    body.data = body.data || {};
    body.data.chat_id = chatId;
    body.data.text = text;
    body.data.reply_timeout = body.reply_timeout || 0;
  ...
  uploadFile(file) {
    return new Promise((resolve, reject) => {
      request({
        ...
        request({
          method: "POST",
          uri: response.data.upload_url,
          headers: {
            "content-type": "x-www-form-urlencoded",
            "bot-key": this.token,
            "file-id": response.data.file_id,
            "hash-send-file": response.data.hash_send_file
          },
          json: true,
          body: file
        }).then(res => {
          if (res.status === "OK") {
            return resolve(response.data.file_id)
          }
          return reject(res)
        }).catch(err => {
          reject(err)
