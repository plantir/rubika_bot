...
class RubikaBotPolling {
  /**
   * Handles polling against the Rubika servers.
   * @param  {RubikaBot} bot
   * @see https://core.rubika.org/bots/api#getting-updates
   */
  constructor (bot) {
    this.bot = bot
    this.options =
      typeof bot.options.polling === 'boolean' ? {} : bot.options.polling
    this.options.interval =
      typeof this.options.interval === 'number' ? this.options.interval : 300
    this.options.params =
      typeof this.options.params === 'object' ? this.options.params : {}
    this.options.params.data = {
      start_message_id: bot.options.start_message_id || 0,
      limit: bot.options.limit || 100
    }
   ...
   */
  _polling () {
    this._lastRequest = this._getMessages()
      .then(updates => {
        this._lastUpdate = Date.now()
        updates.forEach(update => {
          this.options.params.data.start_message_id = +update.message_id + 1
          try {
      ...
      .catch(err => {
         ...
         * the process).
         * See https://github.com/yagop/node-rubika-bot-api/issues/36#issuecomment-268532067
         */
        ...
        return this.bot
          ...
          .catch(requestErr => {
            ...
            const bugUrl =
              'https://github.com/yagop/node-rubika-bot-api/issues/36#issuecomment-268532067'
            console.error(
              'error: Internal handling of The Offset Infinite Loop failed'
            )
            console.error(`error: Due to error '${requestErr}'`)
            console.error(
              'error: You may receive already-processed updates on app restart'
            )
   ...
   */
  _getMessages () {
    return this.bot.getMessages(this.options.params).catch(err => {
      if (err.response && err.response.statusCode === ANOTHER_WEB_HOOK_USED) {
        return this._unsetWebHook().then(() => {
          return this.bot.getMessages(this.options.params)
        })
      }
      throw err
    })
...
module.exports = RubikaBotPolling
